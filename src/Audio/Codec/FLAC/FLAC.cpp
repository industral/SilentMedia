...
SilentMedia::Codec::FLAC::FLAC(void) :
  decoder(NULL), pdecoder(NULL),
      iterator(new ::FLAC::Metadata::SimpleIterator), playCheck(false),
      seekCheck(false), fileSize(-1), streamSize(-1), avrBitrate(-1),
      totalSamples(0), totalTime(-1), curr_sample(0),
      position(new FLAC__uint64), streamInfo(
...
FLAC__StreamDecoderWriteStatus write_callback(const FLAC__StreamDecoder * dec,
  ...
  SilentMedia::Codec::FLAC * flacObj =
      static_cast<SilentMedia::Codec::FLAC *> (client_data);
...
bool SilentMedia::Codec::FLAC::templateInitFile(FLAC__StreamDecoder * dec) {
  ...
  FLAC__StreamDecoderInitStatus retCode = FLAC__stream_decoder_init_file(dec,
      this -> fileName.c_str(), write_callback, metadata_callback,
      error_callback, this);
  ...
  case FLAC__STREAM_DECODER_INIT_STATUS_UNSUPPORTED_CONTAINER:
    std::cout << "Error in FLAC__stream_decoder_init_file(): "
  ...
  case FLAC__STREAM_DECODER_INIT_STATUS_INVALID_CALLBACKS:
    std::cout << "Error in FLAC__stream_decoder_init_file(): "
        << "A required callback was not supplied" << std::endl;
    return false;
    break;
  case FLAC__STREAM_DECODER_INIT_STATUS_MEMORY_ALLOCATION_ERROR:
    std::cout << "Error in FLAC__stream_decoder_init_file(): "
        << "An error occurred allocating memory" << std::endl;
    return false;
    break;
...
void SilentMedia::Codec::FLAC::getPicture(void) {
  ...
  if (::FLAC::Metadata::get_picture(this-> fileName . c_str(), picture,
    ...
    picData . insert(std::pair<::FLAC__StreamMetadata_Picture_Type,
        std::string>(picture . get_type(), spdata));
...
void SilentMedia::Codec::FLAC::setPicture(std::string coverData) {
  ...
  std::string mime = "image/" + Utils::Func::toLower(std::string(
      imagick -> magick()));
...
void SilentMedia::Codec::FLAC::parseMetaData(void) {
  ...
  this -> totalTime = (FLAC__stream_decoder_get_total_samples(this -> decoder)
      / this -> streamInfo -> get_sample_rate());
